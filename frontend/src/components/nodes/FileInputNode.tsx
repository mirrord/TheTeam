import { Handle, Position } from 'reactflow'
import { FileInput, Play, Edit } from 'lucide-react'
import { useState } from 'react'

export interface FileInputNodeData {
  label: string
  file_path?: string
  save_to?: string
  encoding?: string
  type: 'fileinput'
  isStartNode?: boolean
  isEndNode?: boolean
  onEdit?: () => void
}

export function FileInputNode({ data }: { data: FileInputNodeData }) {
  const [isHovered, setIsHovered] = useState(false)
  
  return (
    <div 
      className="px-4 py-3 shadow-lg rounded-lg border-2 border-teal-500 bg-gray-800 min-w-[200px] transition-all hover:shadow-xl hover:border-teal-400 relative"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Edit button */}
      {data.onEdit && (
        <button
          onClick={(e) => {
            e.stopPropagation()
            data.onEdit?.()
          }}
          className="absolute top-2 right-2 p-1 bg-gray-700 hover:bg-gray-600 rounded opacity-0 hover:opacity-100 transition-opacity z-10"
          style={{ opacity: isHovered ? 1 : 0 }}
        >
          <Edit className="w-3 h-3 text-gray-300" />
        </button>
      )}

      <div className="flex items-center gap-2 mb-2">
        <FileInput className="w-4 h-4 text-teal-400" />
        <div className="font-bold text-sm text-gray-100 flex-1">{data.label}</div>
        {data.isStartNode && (
          <div className="flex items-center gap-1 text-xs px-2 py-0.5 bg-green-600 rounded-full">
            <Play className="w-3 h-3" />
            <span>START</span>
          </div>
        )}
      </div>

      <div className="text-xs text-teal-300 font-semibold mb-1">
        FILE INPUT
      </div>

      {data.file_path && (
        <div className="text-xs text-gray-400 mt-2 max-w-[200px] truncate">
          📁 {data.file_path}
        </div>
      )}

      {/* Show output variable name on hover */}
      {isHovered && (
        <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-full pl-2">
          <div className="text-xs text-teal-400 whitespace-nowrap">
            {data.save_to || 'file_content'}
          </div>
        </div>
      )}

      {/* Output handle for file content */}
      <Handle
        type="source"
        position={Position.Right}
        id="output"
        style={{
          background: '#14b8a6',
        }}
        title={data.save_to || 'file_content'}
      />
    </div>
  )
}
